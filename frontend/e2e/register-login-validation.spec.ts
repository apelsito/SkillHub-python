...
// TC_UN_* username input / TC_EM_* email input / TC_PW_* password input
// TC_REG_* register/login flows / TC_UI_* UI/UX

let existingRegisteredUsername: string | null = null
const DUPLICATE_USERNAME_ERROR = /already.*exist|taken|username.*used/i
const REGISTER_RATE_LIMIT_ERROR = /too many|too frequent|rate limit|too frequent/
...
test.describe('Register - Username Validation (Real API)', () => {
  ...
  test('TC_UN_001: accepts valid username with minimum 3 characters', async ({ page }) => {
    ...
    await expect(page.getByText(/only.*letter|username.*required/i)).not.toBeVisible()
  })
  ...
  // TC_UN_010 P1 - unsupported characters
  test('TC_UN_010: shows error for username containing unsupported characters', async ({ page }) => {
    await page.getByLabel(/username/i).fill('user123')
    await page.getByLabel(/username/i).blur()
    await expect(page.getByText(/letter|number|underscore|alphanumeric/i)).toBeVisible()
  })
})
