...
describe('authApi.logout', () => {
  it('uses the runtime base URL and CSRF header', async () => {
    window.__SKILLHUB_RUNTIME_CONFIG__ = { apiBaseUrl: 'https://api.example.com/skill_hub' }
    Object.defineProperty(globalThis, 'document', {
      configurable: true,
      writable: true,
      value: { cookie: 'XSRF-TOKEN=csrf-token' },
    })
    const fetchMock = vi.fn().mockResolvedValue({
      status: 204,
    })
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://api.example.com/skill_hub/api/v1/auth/logout',
      expect.objectContaining({
        method: 'POST',
        headers: expect.any(Headers),
      }),
...
describe('labelApi', () => {
  it('loads visible labels from the web API route', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
        code: 0,
        msg: 'ok',
        data: [{ slug: 'official', type: 'PRIVILEGED', displayName: 'Official' }],
      }),
    })
    vi.stubGlobal('fetch', fetchMock)

    const labels = await labelApi.listVisible()

    expect(labels).toEqual([{ slug: 'official', type: 'PRIVILEGED', displayName: 'Official' }])
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/web/labels',
      expect.objectContaining({
        headers: expect.any(Headers),
